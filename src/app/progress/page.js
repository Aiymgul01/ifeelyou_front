...
export default function ProgressPage() {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center text-brand-purple text-2xl animate-pulse">
        Деректер жүктелуде...
      </div>
    );
  if (!user)
  ...
  const pieData = [
    {
      name: "Дұрыс",
      value: totalCorrect > 0 ? totalCorrect : 1,
      color: "#5c6bc0",
    },
  ...
  const achievements = [
    {
      id: 1,
      title: "Шапшаң қоян",
      desc: "1-Кезең: Ойынды аяқтау",
      icon: "🐇",
      unlocked: checkAchievement(1, false),
    },
    {
      id: 2,
      title: "Ақылды түлкі",
      desc: "1-Кезең: Қатесіз жауаптар",
      icon: "🦊",
      unlocked: checkAchievement(1, true),
    },
    {
      id: 3,
      title: "Жүйрік жылқы",
      desc: "2-Кезең: Ойынды аяқтау",
      icon: "🐎",
      unlocked: checkAchievement(2, false),
    },
    {
      id: 4,
      title: "Дана үкі",
      desc: "2-Кезең: Жоғары концентрация",
      icon: "🦉",
      unlocked: checkAchievement(2, true),
    },
    {
      id: 5,
      title: "Жылдам леопард",
      desc: "3-Кезең: Сөздерді аяқтау",
      icon: "🐆",
      unlocked: checkAchievement(3, false),
    },
    {
      id: 6,
      title: "Күшті айдаһар",
      desc: "3-Кезең: Максималды деңгей",
      icon: "🐉",
      unlocked: checkAchievement(3, true),
    },
    {
      id: 7,
      title: "Арыстан лидер",
      desc: "Барлық кезеңді аяқтау",
      icon: "🦁",
      unlocked:
        checkAchievement(1, false) &&
        checkAchievement(2, false) &&
        checkAchievement(3, false),
    },
  ];
  const handleNextStage = () =>
    setSelectedStage((prev) => (prev === "3" ? "1" : String(Number(prev) + 1)));
  const handlePrevStage = () =>
    setSelectedStage((prev) => (prev === "1" ? "3" : String(Number(prev) - 1)));
    ...
    <div className="max-w-7xl mx-auto px-10 py-10 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-12">
          ...
          <div className="h-64 w-full">
            {activityData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={activityData}>
                  <XAxis
                    ...
                    axisLine={false}
                    tickLine={false}
                    tick={{ fill: "#5c6bc0", fontSize: 12, fontWeight: 600 }}
                    dy={10}
                  />
                  <Tooltip
                    cursor={{ stroke: "#e0e7ff", strokeWidth: 40 }}
                    contentStyle={{
                      borderRadius: "12px",
                      border: "none",
              ...
              <div className="flex items-center justify-center h-full text-gray-400">
                Әзірге статистика жоқ (Пока нет данных)
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 bg-white rounded-[40px] p-10 shadow-sm border border-gray-50 flex flex-col items-center justify-center relative">
          <h3 className="text-xl font-bold text-brand-purple absolute bottom-10">
            Жалпы нәтиже
          </h3>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-[2] bg-[#f2f4fa] rounded-[40px] p-10 shadow-sm border border-indigo-50 overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-10 bg-white p-2 rounded-full shadow-sm">
            <button
              ...
              className="w-10 h-10 bg-[#eef0f8] rounded-full flex items-center justify-center text-brand-purple font-bold hover:bg-brand-purple hover:text-white transition-colors duration-300"
            >
              {"<"}
            </button>
          ...
          <div
            key={selectedStage}
            className="space-y-6 overflow-y-auto max-h-80 pr-4 custom-scrollbar animate-fade-in"
          >
            {stageHistory.length > 0 ? (
              stageHistory.map((game, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-center gap-6 bg-white p-4 rounded-2xl shadow-sm"
                  >
                    ...
                    <span className="text-brand-purple font-bold w-12 text-right">
                      {percentage}%
                    </span>
                  </div>
                );
              ...
              <div className="text-center py-10">
                <p className="text-gray-400 font-medium">
                  Бұл кезеңді әлі ойнаған жоқсыз.
                </p>
        ...
        <div className="flex-1 bg-[#f2f4fa] rounded-[40px] p-8 shadow-sm border border-indigo-50">
          <h3 className="text-2xl font-bold text-brand-purple mb-6">
            Жетістіктер
          </h3>
            ...
            {achievements.map((ach) => (
              <div
                ...
                className={`p-4 rounded-[20px] flex items-center gap-4 transition-all duration-300 ${
                  ach.unlocked
                    ? "bg-brand-purple text-white shadow-md transform hover:scale-105"
                    : "bg-[#e0e4f5] text-brand-purple/50 grayscale"
                }`}
              >
                <div
                  className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl bg-white shadow-inner ${ach.unlocked ? "" : "opacity-50"}`}
                >
                ...
                <div>
                  ...
                  <p
                    className={`text-xs mt-1 ${ach.unlocked ? "text-indigo-200" : "text-brand-purple/40"}`}
                  >
                    {ach.desc}
                  </p>
                </div>
